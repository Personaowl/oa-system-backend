...
角色是数据库记录，可以在运行时创建、编辑、删除并分配权限，页面访问不再依赖固定角色名称。
数据可见范围同样作为权限配置：

- `data:scope:all`：全部组织数据。
- `data:scope:department`：用户被配置为主负责人或协同主管的部门数据。
- `data:scope:self`：仅当前用户本人数据。

业务角色应选择一个合适的数据范围权限；授权变化后，相关用户重新登录即可生效。
